    ...
    def study_area_select(
        ...
        if table_data_exist and table_mahdoude_exist:
            ...
            mahdoude_data = pd.read_sql_query(
                sql=f'SELECT DISTINCT "MAHDOUDE" FROM {TABLE_NAME_DATA};',
                con=engine
            )
            ...
            mahdoude_layers = pd.read_sql_query(
                sql=f'SELECT DISTINCT "MAHDOUDE" FROM {TABLE_NAME_MAHDOUDE};',
                con=engine_layers
            )
    ...
    def aquifer_select(
        ...
        if study_area_selected is not None and len(study_area_selected) != 0:
            ...
            df_aquifer_data = pd.read_sql_query(
                sql=f"SELECT DISTINCT \"MAHDOUDE\", \"AQUIFER\" FROM {TABLE_NAME_DATA} WHERE \"MAHDOUDE\"='{study_area_selected}';",
                con=engine
            )
            ...
            df_aquifer_layers = pd.read_sql_query(
                sql=f"SELECT DISTINCT \"MAHDOUDE\", \"AQUIFER\" FROM {TABLE_NAME_AQUIFER} WHERE \"MAHDOUDE\"='{study_area_selected}';",
                con=engine_layers
            )
        ...
        if (study_area_selected is not None) and (len(study_area_selected) != 0) and\
            (aquifer_selected is not None) and (len(aquifer_selected) != 0):
                ...
                df_well_data = pd.read_sql_query(
                    sql=f"SELECT DISTINCT \"MAHDOUDE\", \"AQUIFER\", \"LOCATION\" FROM {TABLE_NAME_DATA} WHERE \"MAHDOUDE\"='{study_area_selected}' AND \"AQUIFER\"='{aquifer_selected}';",
                    con=engine
                )
                ...
                df_well_layers = pd.read_sql_query(
                    sql=f"SELECT DISTINCT \"MAHDOUDE\", \"AQUIFER\", \"LOCATION\" FROM {TABLE_NAME_AQUIFER} WHERE \"MAHDOUDE\"='{study_area_selected}' AND \"AQUIFER\"='{aquifer_selected}';",
                    con=engine_layers
                )
                            
                aquifer_layers = df_well_layers['AQUIFER'].values.tolist()
